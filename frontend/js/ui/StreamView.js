 ...
 * that size — the longer side of the pointer's actual ink, not of the 32-pixel
 * canvas Windows pads it into — and it was measured rather than reasoned: 28
 * was the first guess and it read as oversized on a real iPhone, a pointer that
 * covered what it was aiming at; 18 read right while the host still sized the
 * canvas, which put the arrow's ink at about 14, and became oversized again the
 * day the host started sizing the ink itself. So 14 it is, in ink. It is a
 * target and not a floor — the host will not shrink a pointer that is already
 * bigger, so zooming in walks the magnification back down to 1 on its own, and
 * past that the pointer is simply its real size.
 *
 ...
 */
/**
 * How long after the viewer's own pointer moved a shape report from the host
 * still counts as an answer to that move. A round trip plus the host's own
 * cadence, with room to spare: shorter than this and a shape asked for by a
 * slow, deliberate move would be held back until the next one.
 */
...
export class StreamView {
    constructor(
        ...
        // Whether the last beat carried anything. It buys the CLOSING beat:
        // one empty state after a burst, so the host hears "nothing is held"
        // once instead of only ever hearing what is.
        this._lastBeatHadState = false;
        ...
        if (this._activeRendererKind === 'webgpu') {
            enhancerName =
                this._videoEnhancementAlgo === 'off'
                    ...
                    : { fsr1: 'FSR1', nis: 'NIS', sgsr: 'SGSR' }[this._videoEnhancementAlgo] ||
                      'SGSR';
    ...
    _handleStatsMessage(msg) {
        ...
        if (msg.type === 'cursor') {
            ...
            // Repainted now only if this shape is about OUR pointer.
            //
            // The shape follows whatever the host's pointer is over, and the
            // viewer's pointer is not the only thing that moves it: a hand on
            // the host's own mouse moves it too, over other windows, changing
            // the shape reported. Painting that here changed the pointer under
            // a hand that had never moved — an I-beam appearing on a still
            // desktop pointer because someone at the host swept across a text
            // field. So a shape that lands while our own pointer has been
            // still is kept, not shown: it is applied by the next move of the
            // viewer's mouse, which is when it becomes about their pointer
            // again (every mousemove re-reads it — see _applyLocalCursor).
            //
            // The grace is generous on purpose: the host's answer to our own
            // move arrives a round trip later, by which time the hand may
            // already have stopped. And the first shape of a session is always
            // shown — there is no earlier one to keep, and nothing has moved
            // yet by definition.
    ...
    /** Whether the viewer's own pointer has moved recently enough that what the
    ...
    /**
     * The character this keystroke produced on the client, and whether the US
     * layout would have produced something else at the same position.
     *
     * Both halves are needed because the two kinds of host use them
     * differently, and only the backend knows which kind it is talking to
     * (see InputMsg::resolveKey):
     *
     *  - The native host resolves the CHARACTER in its own real layout, so it
     *    wants it for every printable key — including the ones that happen to
     *    agree with US. A US-layout viewer on a French host pressed the key
     *    marked A, meant `q`, and used to get `a`: nothing about that keystroke
     *    diverges from US, and it was wrong all the same. What the position
     *    must be compared against is the HOST's layout, which the client cannot
     *    know — so it stops guessing and always says which character it meant.
     *  - A Sunshine host cannot resolve a character at all; the best it can do
     *    is re-interpret a letter's virtual key in its active layout. That is
     *    worth doing only when the client's character actually differs from the
     *    US one, hence `nonUs`: for everyone else it would swap a deterministic
     *    scancode for one Sunshine derives from its own thread's layout, for no
     *    gain.
     *
     * @param {KeyboardEvent} e
     * @returns {{char: string, nonUs: boolean}|null}
     */
    static clientChar(e) {
        const key = e.key;
        ...
        // position has no character for it at all — so it is non-US by
        // definition. (Windows reports AltGr as Ctrl+Alt, so this has to be
        // decided before anything treats the keystroke as a chord.)
        let altGraph = false;
        try {
        ...
        if (altGraph) return { char: key, nonUs: true };

        // A real Ctrl/Meta chord goes through here unchanged: e.key is still the
        // layout's character, so Ctrl+A on AZERTY reads 'a' and is marked non-US
        // against the 'q' at that position — precisely the case that used to
        // reach the host as Ctrl+Q.
        const us = StreamView.US_LAYOUT_CHARS[e.code];
        // A key US has no character for at all (IntlBackslash, …) is non-US too.
        if (!us) return { char: key, nonUs: true };
        return { char: key, nonUs: key !== us[e.shiftKey ? 1 : 0] };
    }
    ...
    handleKeyDown(e) {
        ...
        // Above the auto-repeat branch on purpose — a held movement key then
        // clears the drift on its own, without the player doing anything.
        ...
        // …and, for a printable key, which character the client's layout put
        // there — plus whether the US layout disagrees. What each host does with
        // that is InputMsg::resolveKey's business, not ours.
        const cc = StreamView.clientChar(e);
        const char = cc ? cc.char : null;
        const nonUs = cc ? cc.nonUs : false;
    ...
    handleKeyUp(e) {
        ...
        this._sendKeyEvent({
            ...
            nonUs: this._heldPhysKeys.get(e.code)?.nonUs ?? false,
            ctrlKey: e.ctrlKey,
            shiftKey: e.shiftKey,
            altKey: e.altKey,
            metaKey: e.metaKey,
    ...
    /**
     * The same keys, each against the DOM flag that says it is being held.
     *
     * CapsLock is deliberately absent. getModifierState('CapsLock') reports
     * the LOCK, not a key being held down, so reconciling against it would
     * release a key the viewer never pressed — and, with the lock on, never
     * let it go up at all.
     */
    static MODIFIER_FLAG_BY_CODE = new Map([
        ['ControlLeft', 'ctrlKey'],
    ...
    /** The DOM modifier-state name behind each of those flags. */
    static MODIFIER_STATE_BY_FLAG = {
        ctrlKey: 'Control',
        shiftKey: 'Shift',
        altKey: 'Alt',
        metaKey: 'Meta',
    ...
    /**
     * Release every modifier we believe is held that this event says is not.
     *
     * A modifier can get stuck on the host for one everyday reason: the local
     * OS eats its key-up. Press the Windows key and the Start menu takes the
     * focus; Alt+Tab and the desktop-switch chords do the same. Blur catches
     * most of it, but not all — and on a macOS host the damage outlives the
     * slip, because there a modifier is not a key the OS holds for us but a
     * flag we put on every event we send. Command left down turned the Space
     * bar into Cmd+Space: Spotlight opened instead of the character jumping.
     *
     * Every keyboard event states the whole modifier state, so the truth is
     * right there on each one. Run it before anything else looks at the
     * event and a stuck modifier lasts exactly one keystroke — in a game,
     * where a movement key is repeating, that is a few dozen milliseconds
     * and the player never sees it.
     *
     * The released key is NOT added to _swallowKeyUpCodes: a stale token
     * would eat the real key-up of the NEXT press of that modifier, which is
     * the very bug this method exists to clear. An unmatched key-up costs
     * nothing on any of the three hosts.
     */
    _reconcileModifiers(e) {
        if (!this._heldPhysKeys || this._heldPhysKeys.size === 0) return;
        // Snapshot: _sendKeyEvent deletes from the map we are iterating.
        for (const [id, payload] of [...this._heldPhysKeys]) {
            // Toolbar-latched modifiers have numeric ids and no physical key
            // behind them: they are held on purpose until the user unlatches
            // them, and the DOM event knows nothing about them.
            if (typeof id !== 'string') continue;
            // The key this event is about settles its own state below.
            if (id === e.code) continue;
            const flag = StreamView.MODIFIER_FLAG_BY_CODE.get(id);
            if (!flag) continue;
            if (StreamView.modifierHeld(e, flag)) continue;
            this._sendKeyEvent({
                type: 'keyup',
                keyCode: payload.keyCode,
                code: payload.code,
    ...
    /**
     * Whether @p e says the modifier behind @p flag is down.
     *
     * getModifierState is the normative answer and the one that knows about
     * a key held since before this event; the boolean is the fallback for a
     * synthesised event that has no such method (the test harness, and the
     * soft keyboard's own events).
     */
    static modifierHeld(e, flag) {
        const name = StreamView.MODIFIER_STATE_BY_FLAG[flag];
        if (name && typeof e.getModifierState === 'function') return !!e.getModifierState(name);
        return !!e[flag];
    }
     ...
     * and one CLOSING beat after a burst.
     *
     * That closing beat is what repairs a key the host believes is down and
     * we do not: an empty state makes the host's watchdog release everything
     * it still holds. Without it the heartbeat only ever speaks about keys
     * that ARE held, so a phantom one — a modifier whose key-up the local OS
     * ate — was never contradicted, and on a macOS host, where a modifier is
     * a flag we put on every event rather than a key the OS holds for us,
     * Command stayed down for the rest of the stream.
     *
    ...
    _sendInputState(force = false) {
        ...
        const hasState = held.size > 0 || buttonsHeld.size > 0 || padActive;
        if (!force && !hasState && !this._lastBeatHadState) return;
        this._lastBeatHadState = hasState;
