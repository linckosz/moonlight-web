 ...
 * On Intel (oneVPL) and NVIDIA (NVENC) the waves are spaced, one every ~8 s,
 * to keep a still screen light on a slow link: the repair can take up to ~10 s
 * there (the gap plus one wave), and the
...
export class BackendClient {
    ...
    /**
     * "MoonlightWeb Virtual Display" on the server's machine: {supported,
     * installed, enabled, active, can_manage, method, name, job}. Nothing
     * here adds or removes it — the installer does — and opening its card
     * on the hosts page is what turns it on.
     */
