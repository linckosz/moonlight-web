...
How far the correction reaches depends on what the host can do. **The MoonlightWeb host gets it exactly right in every direction**, because it is the only one that can compare your key against its OWN layout — a US keyboard on a French host is corrected just as a French keyboard on a US host is. **A Sunshine host** can only re-interpret a letter's virtual key, so letters are corrected and digits and punctuation stay positional: they are still mistyped when the two layouts differ, exactly as before, because the alternative would inject them as text and take their key state away. Set the key to `false` to send POSITIONS for everything again, the way every Moonlight client has always done. A Linux or Wolf host is positional either way: they type Unicode through the GTK/IBus sequence, which arrives as garbage with no input method listening.
