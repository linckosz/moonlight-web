...
| Native cadence | A vsync client (tearing off) gets a stream cadence that divides its refresh — 72 for 60 on 144 Hz, 55 on 165 Hz — so frames land on its grid instead of beating against it (September 2026; `CadenceAlign.h`, client measures its refresh by rAF). Mid-session re-choice on a monitor change is wired but browsers keep their rAF clock on one screen, so it rarely fires | Confirm the mid-session path on a real multi-monitor client |
| Native recovery | With a host that heals a named loss by a delta, the browser no longer asks for a keyframe on its own account — a silence, an incomplete or a stale frame are counted, not answered (September 2026); the ride-out watchdog counts received frames against the wave's period (`intra_refresh_frames`) instead of 2.5 s; a requested keyframe on a still screen goes out at once instead of at the next 500 ms floor tick; a tab back from the background re-primes its timing and repeats its pads | Frozen page: the host's governor still cuts −20 % for the 4 s of missing reports and takes five reports to come back — a "resume" fast path is not written |
| Native as a service | The standard install is a Windows service (session 0, no desktop): the native engine is probed and its stream worker launched into the **console session** as the logged-on user (`CreateProcessAsUser` on `winsta0\default`, the user's linked/elevated token — never SYSTEM), the probe cached as a snapshot renewed on logon/logoff; `/api/native/status` reports it (September 2026, [ch. 9 §15](../design/native-capture-encoder.md)) | Verified in `MW_CONSOLE_LAUNCH=force` mode; a full native stream through the console token, and Sunshine/Wolf non-regression on the moved launcher, are Bruno's real-service test |
| Windows installer without Sunshine | The Inno wizard no longer detects, downloads, credentials or pairs Sunshine: the machine streams itself, so it ships one binary and no plaintext password on disk ([§9.1](09-Installers-and-Packaging.md)). `SunshineInstaller` and the in-app `SetupView` step are untouched — that wizard is macOS/Linux only, and those are precisely the platforms with no native backend until Phase I | Compiled locally on 2026-09-07 (Inno Setup 6.7.3, portable): a clean `Successful compile`, 24.8 MB installer; the CI installers job remains the check for the release payload |
| Native host audio | The `mute_host_audio` stream setting now reaches the native host (2026-09-07, `HostMute.h`): an output that mutes **in hardware** is muted for the session while the WASAPI loopback keeps hearing the mix (measured: RMS unchanged, speakers silent); an output that mutes in software would silence the capture too, so the session routes the default output to a virtual sink when one exists (Steam Streaming Speakers, VB-Cable, VoiceMeeter…) and otherwise leaves the speakers on, saying why in the log ([ch. 9 §24](../design/native-capture-encoder.md)) | Only Windows; macOS and Linux hosts still play their own audio. A worker that dies mid-session leaves the mute in place (lift it in the Sound panel) |
| **Linux native host** | Usable end to end since September 2026 — picture, sound, keyboard, mouse, pad, healing without a keyframe, and a **ScreenCast portal route** for a machine that may not read its own scanout (an AppImage): the consent is stored and replayed, so the dialog is one click per *installation* ([ch. 9 §19.15](../design/native-capture-encoder.md)) | What a Linux host still cannot do, in the order a user notices ([§19.16](../design/native-capture-encoder.md)): **(1)** muting the host's own speakers is unwritten — `HostMute` is Windows-only, so `mute_host_audio` is silently ignored and the game plays in the room; the way in is a null PipeWire sink, not a driver; **(2)** the consent's *return* trip (a **new** grant reaching `settings.json`) is written and unit-tested but needs a human click on a real dialog to be seen; **(3)** AV1 is written and ⛔ refused by Mesa 23.2 — reopen on a newer driver; **(4)** the multi-monitor absolute pointer fix has never run on a real two-screen Linux host; **(5)** HDR does not exist at all (`hdrActive = false`, 8-bit capture); **(6)** ⛔ Wayland will never allow reading or moving another client's pointer — by protocol design, not a gap to fill |
...
- HDR on the native host (FP16 capture → P010 → HEVC Main10 / AV1 10-bit) — shipped on Windows, macOS and Intel; **Linux has none of it**, and no bench can judge it without an HDR panel on the machine itself.
- Muting the host's speakers on macOS and Linux. Windows has it (hardware mute, else a virtual sink); on Linux the sound is already taken from a PipeWire sink monitor, so the answer is a null sink the session's playback is moved to — ⚠️ never a volume mute, which would silence the capture with the speakers.
