...
| Native as a service | The standard install is a Windows service (session 0, no desktop): the native engine is probed and its stream worker launched into the **console session** as the logged-on user (`CreateProcessAsUser` on `winsta0\default`, the user's linked/elevated token — never SYSTEM), the probe cached as a snapshot renewed on logon/logoff; `/api/native/status` reports it (September 2026, [ch. 9 §15](../design/native-capture-encoder.md)) | Verified in `MW_CONSOLE_LAUNCH=force` mode; a full native stream through the console token, and Sunshine/Wolf non-regression on the moved launcher, are Bruno's real-service test. Sunshine is not yet removed from the installer (waits on that) |
