...
There is **no compile-only build stage**: packaging compiles the same four targets and additionally exercises `cmake --install`, `windeployqt`, the Inno Setup script, linuxdeploy and `macdeployqt` — the parts a bare build never touches, and where breakage actually happens. To test any commit, open its run and download the `<App>-windows-<arch>-<label>` / `<pkg>-linux-x64-<label>` / `<pkg>-macos-arm64-<label>` artifact (GitHub serves each as a zip). A manual dispatch picks a **channel**: `dev` (default, `MoonlightWebDev`, version `<last v* tag>-<3-char sha>-dev`, e.g. `0.2.4-e8f-dev`) or `staging` (the PROD edition, `<last v* tag>-<3-char sha>-stg`, e.g. `0.3.0-b7c-stg` — a hidden production build that installs over the release and publishes nothing). `<label>` is `v<version>` on every channel (`v0.3.0-b7c-stg`): only a pushed `v*` git tag releases, an artifact name never does. The version baked into the packages stays bare, since dpkg wants a leading digit. From the command line: `gh workflow run ci.yml --ref main -f channel=staging`.
...
- **setup** job computes the version (feeds `MW_VERSION`): the tag when the run's ref is a `v*` tag, otherwise `<last v* tag>-<3-char sha>-dev` or `-stg` depending on the `channel` input, plus `label` for the artifact names (`v<version>` on a tag, the bare version otherwise). It also exports `pkg_version`, the same string with `-` → `.`, because rpm rejects dashes in `Version` — the Linux job uses that one throughout.
