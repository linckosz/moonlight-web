...
**Not indexed — anything but the presentation website**: `stream.`, `stream.dev.`, `api.`, `dnsapi.`, `updates.` and `stats.` all import `(no_index)`, which adds `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet, noimageindex` to every response and answers `robots.txt` with `Disallow: /` for everyone, naming the AI crawlers explicitly. The `robots.txt` is answered inside a `handle /robots.txt`: a bare `respond` runs after every `handle`, so on `stream.` the entry page's catch-all would have answered it with the bootstrap page. The bootstrap page and the application's own `index.html` also carry `<meta name="robots" content="noindex, nofollow">` — the former covers its GitHub Pages copy, where no header can be set; the latter a host reached on its LAN address or a legacy public name. Only the apex (and `www.`, which redirects to it) is indexable. Be precise about the limit: the names are not secrets — they are in public DNS and in the Certificate Transparency logs from the moment their certificates are issued — they are merely kept out of search results and training sets by robots that obey.
