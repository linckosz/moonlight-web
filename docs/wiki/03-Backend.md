...
- **Four router holes, and they are checked.** When the line first comes up the tunnel maps four UDP+TCP ports and pins one connection to each, then advertises the mapped port under the router's public address — without that, the only address it can offer is a reflexive one the router opened towards the STUN server, which any peer-address-filtering box drops. The candidates are `3478-3481` then `5349-5352` (STUN/TURN and its TLS half): ports a corporate firewall lets out, where it drops arbitrary high ones, and the destination port is what has to be acceptable. A port another machine on the LAN already holds is **left alone**, the entry is **read back after every write and every renewal**, and one that turns out to point elsewhere is dropped rather than advertised. Two MoonlightWeb hosts on one LAN otherwise ask for the same four entries, and a router repoints them silently — the first host keeps offering an address that now reaches the second, its tunnel still works over a permissive network, and it fails on exactly the corporate networks these ports were chosen for (seen 2026-09-09). Without an IGD, or with every candidate taken, connections fall back to an ephemeral port and their reflexive address.
