...
2. **Internet Link page** — explicit opt-in checkbox (unchecked by default) to allow the Internet link (per-session UPnP mapping; the consent text says what is and is not done).
...
4. Drops a **`provisioning.json`** next to the exe; the server consumes it once on first run (`Provisioning::applyOnce`): enable Internet Access (the consent is recorded as `source:"installer"`, versioned, and only when none exists yet — an upgrade that skipped the page cannot overwrite the words originally agreed to), create/pair the local Sunshine via its REST API (`/api/pin`).
5. Runs the server during a **live provisioning checklist** page (Sunshine install → pairing → Internet link — the async step is fed back by the server through a status file; it closes in seconds on a fresh install, and only a legacy upgrade still waits on DNS + ACME), then offers to open the admin page at the URL the server actually serves.
...
Internet Access reaches parity with the `.deb` through build-stage `ARG`s (`MW_DOMAIN`, `MW_PDNS_*`, `MW_ZEROSSL_*`) that CMake reads from the environment at configure time. They live in the build stage only — the runtime stage has its own `FROM`, so nothing lands in the published layers or in `docker history` (verified). The values still end up compiled into the binary, which is exactly the exposure every distributed package already has.
...
| Self-healing shortcuts | The server rewrites the Desktop entry on startup and whenever the entry URL changes (port parity rebind, a legacy domain coming up, the NAT-hairpin verdict flipping) — the installer can't know the runtime port/domain. A fresh install's entry URL stays on `https://localhost`. |
