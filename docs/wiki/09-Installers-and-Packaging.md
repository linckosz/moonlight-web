...
| Self-healing shortcuts | The server rewrites the Desktop entry on startup and whenever the entry URL changes (port parity rebind, a legacy domain coming up) — the installer can't know the runtime port/domain. A fresh install's entry URL stays on `https://localhost`. |
