...
**5. Internet Access — optional.** With a custom domain it registers nothing and requests no certificate; it re-detects the public IP, tests NAT hairpin, and allows the per-slot media mapping (`RouterPortAllocator::enabled()` = `upnp_enabled && internet_access_enabled`). Turn it off and the 48010+ mappings become manual too. `GET /api/internet/status` reports `custom_domain: true` in that mode.
