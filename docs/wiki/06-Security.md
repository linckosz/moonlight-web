...
Three states, in `AppSettings`: **default** (remote admin on, no digest stored), **custom** (a digest stored), **disabled** (`remote_admin_enabled: false`, nothing is accepted).

- **Ships enabled, on the built-in default `moonlightweb`.** The only way to set a password is from the host — and an unreachable host is the entire reason this door exists, so shipping it closed would make the feature useless to the people who need it. The default is documented, therefore **not a secret**: what stands between it and an attacker is that the unlock only ever *promotes an already-authenticated LAN session*. The admin page carries a warning banner until the operator picks their own, because until then **any device granted streaming access can also administer the server**.
- **Changed from the admin page** (`POST /api/admin/password`, admin-gated). Stored as **PBKDF2-SHA256, 210 000 iterations, 16-byte random salt** — the plaintext is never written, so a forgotten password is *reset*, never recovered. Minimum 8 characters, and the built-in default itself is refused (accepting it would clear the banner while leaving the public password in force).
