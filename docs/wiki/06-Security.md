...
- **The clear pair lives in memory only.** So the owner can reopen the popin and read the link and PIN they already sent, `ShareManager` keeps them in the activation — but `share.json` receives digests and nothing else (`test_clear_secrets_are_memory_only`). A stolen data directory therefore yields no usable invitation, and a restart forgets the pair while the invitation itself stays valid: `POST /api/share/slots/:n/credentials` then answers `{available:false}` and the popin says to re-share.
