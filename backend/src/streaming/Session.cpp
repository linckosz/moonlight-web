...
void StreamSession::onLaunchResult(bool ok, const BackendError& err, const MediaDescriptor& media)
{
    ...
    if (media.type == MediaType::NativeHost) {
        ...
        nativeParams.matchClientDisplay = m_MatchClientDisplay;
        nativeParams.fallbackWidth = m_FallbackWidth;
        nativeParams.fallbackHeight = m_FallbackHeight;
...
void StreamSession::onShimConnectionStarted()
{
    ...
    // Native host only, and only when its encoder really does it.
    {
        ...
        // The encoder's name alone — "NVENC", "AMF", "oneVPL" — for the stats
        // overlay. Empty for every other host, whose encoder runs on a machine
        // this process cannot see into. Deliberately not describeSession(): its
