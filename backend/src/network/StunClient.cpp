...
bool StunClient::detectPublicIp(const QList<StunServer>& servers, int timeoutMs, QString& publicIp)
{
    for (const auto& server : servers) {
        qInfo() << "[StunClient] Trying STUN server:" << server.host << ":" << server.port;

        if (queryServer(server, timeoutMs, publicIp)) {
            qInfo() << "[StunClient] Detected public IP:" << publicIp << "from" << server.host
                    << ":" << server.port;
            return true;
        }
...
bool StunClient::queryServer(const StunServer& server, int timeoutMs, QString& publicIp)
{
    ...
    if (!socket.waitForConnected(timeoutMs)) {
        qWarning() << "[StunClient] Connection timeout after" << timeoutMs << "ms to" << server.host
                   << ":" << server.port;
        return false;
    }
    ...
    if (sent != request.size()) {
        qWarning() << "[StunClient] Failed to send STUN request to" << server.host << ":"
                   << server.port << "— wrote" << sent << "of" << request.size() << "bytes";
        return false;
    }
    ...
    if (!socket.waitForReadyRead(timeoutMs)) {
        qWarning() << "[StunClient] Read timeout waiting for STUN response from" << server.host
                   << ":" << server.port << "(timeout:" << timeoutMs << "ms)";
    ...
    if (response.isEmpty()) {
        qWarning() << "[StunClient] Empty response from" << server.host << ":" << server.port;
        return false;
    }
...
QByteArray StunClient::buildBindingRequest()
{
    ...
    for (int i = 0; i < 12; ++i) {
        data[8 + i] = static_cast<char>(QRandomGenerator::global()->bounded(256));
    }
                                          ...
                                          const QByteArray& transactionId)
{
    ...
    quint16 type = (static_cast<quint8>(response[0]) << 8) | static_cast<quint8>(response[1]);
    ...
    quint32 cookie = (static_cast<quint8>(response[4]) << 24) |
                     ...
                     (static_cast<quint8>(response[6]) << 8) | static_cast<quint8>(response[7]);
    ...
    int messageLength = (static_cast<quint8>(response[2]) << 8) | static_cast<quint8>(response[3]);
    int pos = kStunHeaderSize;
    int end = pos + messageLength;

    while (pos + 4 <= end && pos + 4 <= response.size()) {
        quint16 attrType =
            (static_cast<quint8>(response[pos]) << 8) | static_cast<quint8>(response[pos + 1]);
        quint16 attrLength =
            (static_cast<quint8>(response[pos + 2]) << 8) | static_cast<quint8>(response[pos + 3]);

        pos += 4;

        // Clamp attrLength to remaining data
        int avail = response.size() - pos;
        if (attrLength > avail) attrLength = static_cast<quint16>(avail);
