...
private:
    ...
    // Reentrancy guard: handleSubmitPin runs a nested Qt event loop (up to 60s);
    // the single-threaded HTTP server would otherwise dispatch the frontend's
    // poll POSTs reentrantly onto the same NvPairingManager → use-after-free.
    QSet<QString> m_SubmitInFlight; // uuid of hosts with a submit in progress
