...
**Internet access is opt-in and off by default.** Enabling it opens ports on your
router via UPnP and publishes a subdomain with a TLS certificate. Certificates
issued by a publicly trusted CA are recorded in public Certificate Transparency
logs, so **the existence of an instance and its hostname are public information**,
permanently. That is inherent to publicly trusted TLS, not specific to this
project. If you would rather not be listed, do not enable internet access — use a
LAN address or a VPN/mesh network instead.
...
- A self-signed certificate warning on a LAN address. Expected until internet
  access is enabled.
