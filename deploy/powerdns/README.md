...
- **mw-proxy** — a small dependency-free Go gateway (built from `mw-proxy/`). It
  holds the real PowerDNS key and lets a MoonlightWeb 2.0.0+ instance manage
...
On the MoonlightWeb server (2.0.0+), set in its own `.env` the **restricted**
...
| `MW_PDNS_PROXY_KEY` | DNS box + app | The **restricted** key. Set it as `MW_PDNS_TOKEN` on the MoonlightWeb 2.0.0+ server. |
...
> the full key, 2.0.0+ → `dnsapi.` with the restricted key. A later release
