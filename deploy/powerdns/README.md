...
- **mw-proxy** — a small dependency-free Go gateway (built from `mw-proxy/`). It
  ...
  **only its own** A record (+ `_owner` / `_acme-challenge` TXT), authenticating
  ...
  [Update relay](#update-relay--installed-version-census) — and it carries the
  code for the **session census**, which is *not* deployed: see
...
`/v1/session`) is where that would be collected — **it is not deployed**. The
code ships, instances know how to report to it, and nothing answers: there is no
`metrics` A record and no site block for it in the Caddyfile. Read the rest of
this section as a description of what turning it on would collect.

MoonlightWeb **0.3.0+** POSTs a small JSON body —
...
**Turning it on** — four steps, and the **order matters**: publish the name
before Caddy is asked to certify it, or it will order a certificate for a name
that does not resolve, fail, and retry forever.

1. Add the A record: `pdnsutil add-record {MW_DOMAIN} metrics A {MW_PUBLIC_IP}`
   then `pdnsutil rectify-zone {MW_DOMAIN}` (a DNSSEC zone serves nothing until
   it is rectified). Check it resolves from outside before continuing.
2. Add the site block to `caddy/Caddyfile.tmpl` — it belongs where the comment
   marks its place, next to `updates.`:
   ...
   Then `docker compose up -d --build caddy` — the template lives *in* the
   image, so a restart alone changes nothing.
3. In Umami, add a **third website**: name `MoonlightWeb sessions`, domain
   `metrics.{MW_DOMAIN}`. Its own website on purpose — there, "views" means
   "sessions started" and nothing else is mixed in.
4. Put its ID in `.env` → `MW_UMAMI_SESSIONS_WEBSITE_ID=<id>` (**append** it;
...
| `MW_SUBDOMAIN_REGISTRATION` | DNS box only | `off` refuses NEW sub-domains; existing ones keep working. See below. |
...
## Retiring the sub-domains

Per-machine sub-domains are being wound down. The wind-down has two dates and
they are not the same one:

| When | What happens | What does not |
|---|---|---|
| **The day 0.3.0 is published** | No NEW sub-domain can be created | Every existing one keeps resolving, updating and renewing |
| **February 2027** | The service stops | — |

The first date is this switch, in `.env`:

```
MW_SUBDOMAIN_REGISTRATION=off
docker compose up -d --force-recreate mw-proxy
```

Startup says which way it went — `sub-domain registration CLOSED` or
`sub-domain registration open`.

**Throw it only once 0.3.0 is actually published.** 0.2.4 stays downloadable
until the shutdown and people will keep installing it; refusing them a
sub-domain before there is a release that reaches machines another way would
leave them with no internet path at all. A refused instance is told so on its
own admin page, in words: this way is retired, 0.3.0 reaches machines through
the introduction service, update to use internet access, and streaming over the
local network is unaffected.

**What "existing" means here.** Not "has an owner recorded" — it is "has an A
record in the zone". Ownership is recorded on first write, so it is absent for
every name registered before that mechanism existed; reading a missing owner as
"this is new" would refuse exactly the installs the retirement promises to keep
serving. If PowerDNS cannot be asked, the request is **allowed**: a refusal is a
dead end for someone who cannot do anything about it, and an outage must not
become one.
